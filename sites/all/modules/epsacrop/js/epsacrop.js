(function($) {
  Drupal.behaviors.EPSACrop = {
    attach: function(context, settings) {
      if(typeof Drupal.settings.EPSACrop === 'object'){
        var type_name = Drupal.settings.EPSACrop.entity_type;
        var field_name = Drupal.settings.EPSACrop.field_name;
        var bundle = Drupal.settings.EPSACrop.bundle;
        var fid = Drupal.settings.EPSACrop.fid;
        var img = Drupal.settings.EPSACrop.url;
        var trueSize = Drupal.settings.EPSACrop.size;
      }
      Drupal.EPSACrop = {
        api: null,
        preset: null,
     ...
     /*$('#edit-field-image-irs-und-0-remove-button').ajaxComplete(function(e, xhr, settings)
      {
        //console.log(settings.url);
        var url = settings.url;
        var type = settings.type;
        if (url.indexOf('/file/ajax/field_image_irs/und/0') > -1 && type=='POST') {
          console.log("img="+img);
          Drupal.EPSACrop.dialog('node', 'field_image_irs', 'irs', fid, img, trueSize);
        }
      });*/
    }
  };
